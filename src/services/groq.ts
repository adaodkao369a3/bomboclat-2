...
function isGroqResponse(value: unknown): value is GroqResponse {
  ...
  if (!firstChoice || typeof firstChoice !== 'object' || !('message' in firstChoice)) return false;
...
export async function generateText(prompt: string, systemInstruction?: string): Promise<string | null> {
  if (!GROQ_API_KEY) {
...
export async function generateClipSummary(messages: Array<{ author: string; content: string }>): Promise<{ title: string; summary: string } | null> {
